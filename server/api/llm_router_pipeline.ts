          ...
          try {
            ...
            if (validationResult.success) {
              ...
              if (simplifiedResult.success) {
                ...
                log('Both schema validations failed. Errors: ' + 
                  fromZodError(simplifiedResult.error).message, 'llm-router-pipeline');
                ...
                if (parsedJson.main_response && typeof parsedJson.main_response === 'string') {
                  log('Falling back to basic main_response validation', 'llm-router-pipeline');
                  return res.json({
                    message: { 
            ...
            log('Error during Zod validation: ' + zodError, 'llm-router-pipeline-error');
          }
        }
        // If we couldn't extract valid JSON or validation failed completely
        log('Failed to parse or validate structured output, returning raw response', 'llm-router-pipeline');
      } catch (parseError) {
        log('Error parsing JSON from LLM response: ' + parseError, 'llm-router-pipeline-error');
